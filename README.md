...
> **Status:** early, developed in private. The engine is battle-tested — it runs the
> financial close of a production operating agent (multi-channel reconciliation across
> Shopify + Amazon + Zoho, hundreds of orders collapsed to a handful of exceptions in
> one agent tool call). This repo is the standalone home for that engine.

## The bigger idea — bring your own data, bring your own agent

Databricks Genie puts an ontology/graph over *their* lakehouse. weave inverts that: a
lightweight, agent-native graph layer an agent stands up over **whatever sources a
company already has** — no platform to own, no data to migrate. Point it at a few
connectors, describe how the records relate (or let an agent propose it), and the agent
reads the whole business as one connected thing. It's the substrate for a
*bring-your-own-data, bring-your-own-agent* data layer — starting small, for teams who
aren't large enterprises.

See [`docs/agent-configurable-graph.svg`](docs/agent-configurable-graph.svg) for how the
ontology / relationships / rules tiers are meant to become agent-definable.
